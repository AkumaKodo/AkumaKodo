            ...
            if (this._errors < 1) {
                this.container.logger.debug(
                    ...
                    `Saved ${this.uniqueFilePathCounter} ${
                        this.uniqueFilePathCounter > 1 ? "files" : "file"
                    }!`,
                );
            }
        } catch (e) {
    ...
    public async load() {
        try {
            ...
            await import(
                ...
                }/fileloader.ts#${this.uniqueFilePathCounter}`
            );
            this.savedPaths = [];
    ...
    public async fastLoader(
        ...
        try {
            await Promise.all(
                [...paths].map((path) => {
                    if (between) {
                        between(path, this.uniqueFilePathCounter, paths);
                    }
                    this.import(path);
                }),
            );
