...
export class AkumaKodoMongodbProvider extends AkumaKodoProvider {
    protected metadata: AkumaKodoCollection<
        bigint,
    ...
    public get(id: bigint, key: string, defaultValue: any) {
        ...
        if (data) {
            this.logger.debug(
                "info",
                "Mongo Provider get",
                `Fetched value from cache. Guild: ${id} Key: ${key} Value: ${
                    data.settings[key]
                }`,
            );
        ...
        this.logger.debug(
            "warn",
            "Mongo Provider get",
            `Failed to get the value for ${id}. Returning default value.`,
        );
    ...
    public async set(id: bigint, key: string, value: any) {
        ...
        await (await this.instance).updateOne({ guildId: id }, {
        ...
        this.logger.debug(
            "info",
            "Mongo Provider set",
            `Successfully set ${key} to ${value} for guild ${id}`,
        );
    }
    ...
    public async delete(id: bigint, key: string) {
        ...
        this.logger.debug(
            "info",
            "Mongo Provider delete",
            `Deleted ${key} from guild ${id}`,
        );
    }
    ...
    public async clear(id: bigint) {
        ...
        this.logger.debug(
            "info",
            "Mongo Provider clear",
            `Cleared all data for guild ${id}`,
        );
    }
}
